...
all_p = cat(1, all_x, all_y, all_z)';
dim2 = size(all_p, 2);
    
DB.feature.division = [dim1, dim2];
DB.feature.all = cat(2, DB.feature.all, all_p);
DB.feature.dim = size(DB.feature.all, 2);

if sum(DB.opt.dissim_method) == 2
    DB.feature.method{1} = 'euclidean';
    DB.feature.method{2} = 'cosine';
elseif sum(DB.opt.dissim_method) == 1
...
if DB.opt.time
    DB.duration.GaitPoseConjunction = datetime - DB.time.end_time;
    [h, m, s] = hms(DB.duration.GaitPoseConjunction);
    fprintf('[GaitPoseConjunction] processing time : %2.0fh:%2.0fm:%2.0fs \n', fix(h), fix(m), fix(s));
